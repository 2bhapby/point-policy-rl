...
def observation_to_state(
    ...
    points_arr = np.asarray(obs[point_key], dtype=np.float32)
    if points_arr.ndim == 1:
        if (points_arr.size % 3) != 0:
            raise ValueError(
                f"{point_key} size must be divisible by 3, got shape={points_arr.shape}"
            )
        points_3d = points_arr.reshape(-1, 3)
    else:
        flat = points_arr.reshape(-1, points_arr.shape[-1])
        if flat.shape[1] < 3:
            raise ValueError(
                f"{point_key} last dim must be >=3, got shape={points_arr.shape}"
            )
        points_3d = flat[:, :3]

    points_abs = points_3d.reshape(-1).astype(np.float32)
    points_rel_eef = (points_3d - eef_pos.reshape(1, 3)).reshape(-1).astype(np.float32)

    base_action_7d = np.asarray(base_action_7d, dtype=np.float32).reshape(7)
    if mode == "absolute":
        parts = [points_abs, gripper]
    elif mode == "relative_eef":
        parts = [points_rel_eef, gripper]
    else:
        parts = [points_abs, points_rel_eef, gripper]
    if include_eef_pos:
