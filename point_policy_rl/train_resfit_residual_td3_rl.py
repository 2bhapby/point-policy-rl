...
def _sample_mixed_batch(
    *,
    ...
    if online_batch_size > 0 and len(online_rb) >= online_batch_size:
        parts.append(_move_batch_to_device(online_rb.sample(online_batch_size), device))
...
def _build_resfit_qagent(args: argparse.Namespace, device: str, action_dim: int, prop_dim: int):
        ...
        if (
            ...
            for i in range(num_updates_per_iteration):
                ...
                if effective_sampling_strategy == "prioritized_replay" and "_td_errors" in metrics:
                    ...
                    if offline_fraction > 0.0 and offline_rb is not None:
                        if cur_online > 0:
