...
class Workspace:
    ...
    def eval(self):
        ...
        for env_idx in range(self.envs_till_idx):
                ...
                while True:
                    ...
                    if self.emit_temporal_debug and hasattr(
                        ...
                        if isinstance(temporal_debug, dict):
                            slot_indices = temporal_debug.get("slot_indices", [])
                            weights = temporal_debug.get("weights", [])
                            slot_preview = ",".join(map(str, slot_indices[:8]))
                            weight_preview = ",".join(
                            ...
                            temporal_debug_lines.append(
                                " ".join(
                                    [
                                        ...
                                        f"query_stride={int(temporal_debug.get('query_stride', -1))}",
                                        f"query_offset={int(temporal_debug.get('query_offset', -1))}",
                                        f"num_candidates={int(temporal_debug.get('num_candidates', -1))}",
                                        f"weight_mode={temporal_debug.get('weight_mode', '-')}",
                                        f"slots=[{slot_preview}]",
            ...
            if self.emit_temporal_debug and len(temporal_debug_lines) > 0:
                ...
                with temporal_txt.open("w", encoding="utf-8") as fp:
                    fp.write(
                        ...
                        "slots: where each query prediction was written in all_time_actions\n"
                        "weights: aggregation weights applied to candidates for current step\n\n"
                    )
...
@hydra.main(config_path="cfgs", config_name="config_eval_parallel_v1", version_base=None)
def main(cfg):
