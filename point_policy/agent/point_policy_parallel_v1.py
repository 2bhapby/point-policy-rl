...
class BCAgent:
    def __init__(
        ...
        self.query_stride = int(query_stride) if self.temporal_agg else 1
        self.query_offset = int(query_offset) if self.temporal_agg else 0
        ...
        if self.temporal_weight_mode not in {"recent", "legacy", "uniform"}:
            raise ValueError(
                "temporal_weight_mode must be one of {'recent','legacy','uniform'}, "
    ...
    def buffer_reset(self):
        ...
        if self.temporal_agg:
            self._all_time_actions_width = (
                self.max_episode_len
                + self.query_offset
                + (self.num_queries - 1) * self.query_stride
                + 1
            )
    ...
    def clear_buffers(self):
        ...
        if self.temporal_agg:
            ...
            del self._all_time_actions_width
    ...
    def act(self, obs, norm_stats, step, global_step, eval_mode=False, **kwargs):
        ...
        if not self.temporal_agg:
            ...
            for idx in range(len(future_tracks)):
                ...
                track = track.transpose(0, 1).reshape(self.num_queries, -1)
                slot_indices = []
                for q in range(self.num_queries):
                    slot = step + self.query_offset + q * self.query_stride
                    if slot >= self._all_time_actions_width:
                        break
                    self.all_time_actions[pixel_key][step, slot] = track[q]
                ...
                if len(tracks_for_curr_step) == 0:
                    fallback_slot = min(
                        max(step + self.query_offset, 0),
                        self._all_time_actions_width - 1,
                    )
                    tracks_for_curr_step = self.all_time_actions[pixel_key][
                        step : step + 1, fallback_slot
                    ]
                if self.temporal_weight_mode == "uniform":
                    exp_weights = np.ones((len(tracks_for_curr_step),), dtype=np.float32)
                elif self.temporal_weight_mode == "legacy":
                    exp_weights = np.exp(
                        -self.temporal_weight_k * np.arange(len(tracks_for_curr_step))
                    ).astype(np.float32)
                else:  # recent
                    recency = np.arange(len(tracks_for_curr_step) - 1, -1, -1)
                    exp_weights = np.exp(-self.temporal_weight_k * recency).astype(
                        np.float32
                    )
                exp_weights = exp_weights / max(exp_weights.sum(), 1e-8)
                exp_weights_t = (
                    torch.from_numpy(exp_weights).to(self.device).unsqueeze(dim=1)
                )
                track = (tracks_for_curr_step * exp_weights_t).sum(dim=0, keepdim=True)
                ...
                if self.emit_temporal_debug and idx == 0:
                    self._last_temporal_debug = {
                        ...
                        "query_stride": int(self.query_stride),
                        "query_offset": int(self.query_offset),
                        "slot_indices": slot_indices,
                        "num_candidates": int(len(tracks_for_curr_step)),
                        "weight_mode": self.temporal_weight_mode,
                        "weights": exp_weights.tolist(),
                    }
