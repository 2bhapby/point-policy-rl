...
RIM_BOTTOM_DEFAULT_LOCAL_POINTS = np.array(
    [
        [0.035, 0.0, 0.02],
        [0.0, 0.035, 0.02],
        [-0.035, 0.0, 0.02],
...
class RGBArrayAsObservationWrapper(dm_env.Environment):
    def __init__(
        ...
        max_delta_rot=0.25,
        normalize_delta_action_to_osc=False,
        ...
        self._object_point_mode = str(object_point_mode).strip().lower()
        if self._object_point_mode not in {
            "geom_fps",
            "cal_offset",
            "cal_offset_both",
            "geom_fps_robot_offset",
        }:
            raise ValueError(
                "object_point_mode must be one of "
                "'geom_fps', 'cal_offset', 'cal_offset_both', "
                "'geom_fps_robot_offset', "
                f"got {object_point_mode}"
            )
        self._robot_point_mode = (
            "cal_offset"
            if self._object_point_mode in {"cal_offset_both", "geom_fps_robot_offset"}
            else "site"
        )
        self._max_delta_pos = float(max_delta_pos)
        self._pose_delta_gain = float(pose_delta_gain)
        self._pose_solve_mode = str(pose_solve_mode).strip().lower()
        if self._pose_solve_mode not in {"rigid", "kp0"}:
            raise ValueError(
                f"pose_solve_mode must be 'rigid' or 'kp0', got {pose_solve_mode}"
            )
        self._max_delta_rot = float(max_delta_rot)
        self._normalize_delta_action_to_osc = _to_bool_flag(
            normalize_delta_action_to_osc
        )
        self._controller_output_max_pos = float(controller_output_max_pos)
        self._controller_output_max_rot = float(controller_output_max_rot)
        if self._controller_output_max_pos <= 1e-8:
            raise ValueError(
                f"controller_output_max_pos must be > 0, got {controller_output_max_pos}"
            )
        if self._controller_output_max_rot <= 1e-8:
            raise ValueError(
                f"controller_output_max_rot must be > 0, got {controller_output_max_rot}"
            )
        ...
        self._gripper_close_object_gate = _to_bool_flag(gripper_close_object_gate)
        self._gripper_close_object_threshold = float(gripper_close_object_threshold)
        self._gripper_close_object_point = str(gripper_close_object_point).strip().lower()
        self._real_deploy_mode = _to_bool_flag(real_deploy_mode)
        if self._gripper_close_object_point not in {"tip_mid", "kp0"}:
            raise ValueError(
                "gripper_close_object_point must be 'tip_mid' or 'kp0', "
                f"got {gripper_close_object_point}"
            )
        # In real deployment we do not have simulator GT object geometry.
        # Force-disable simulator-only object distance gating.
        if self._real_deploy_mode and self._gripper_close_object_gate:
            warnings.warn(
                "real_deploy_mode=true: disabling sim-only gripper_close_object_gate.",
                RuntimeWarning,
            )
            self._gripper_close_object_gate = False
        self._gripper_command_fallback_to_tip_distance = _to_bool_flag(
            gripper_command_fallback_to_tip_distance
        )
        self._gripper_reopen_tip_gate = _to_bool_flag(gripper_reopen_tip_gate)
        ...
        self._object_bottom_at_origin = _to_bool_flag(object_bottom_at_origin)
        self._close_approach_remaining = 0

        # Canonical Franka point layout used by original Point-Policy pipeline.
        ...
        self._robot_offsets_calibrated_from_sites = False
        ...
        if (
            ...
            print(
                "[libero_spatial] warning: "
                "gripper_reopen_tip_gate=true but tip indices (1,2) are not present "
                "in robot_point_indices. Disabling reopen tip gate."
            )
            self._gripper_reopen_tip_gate = False

        if self._use_object_points and self._num_object_points != 10:
            raise ValueError(
                f"Expected num_object_points=10 (bowl5 + plate5), got {self._num_object_points}"
            )
        ...
        self._robot_site_ids_full = None

        self._obj_of_interest = list(getattr(self._env, "obj_of_interest", []))
        self._bowl_object_name = None
        self._plate_object_name = None
        self._bowl_body_id = None
        self._plate_body_id = None
        self._bowl_geom_ids = None
        self._plate_geom_ids = None
        self._bowl_local_points = None
        self._plate_local_points = None
    ...
    def _compute_gripper_to_bowl_distance(self, current_robot_points):
        if self._real_deploy_mode:
            return None
        if not self._use_object_points or self._current_raw_obs is None:
            return None
        self._ensure_geometry_metadata(self._current_raw_obs)
        if self._bowl_geom_ids is None or len(self._bowl_geom_ids) == 0:
            return None
        bowl_points = np.asarray(
            self._env.sim.data.geom_xpos[self._bowl_geom_ids], dtype=np.float32
        ).reshape(-1, 3)
        if bowl_points.shape[0] == 0:
            return None
        if self._gripper_close_object_point == "kp0":
            gripper_ref = current_robot_points[0]
        else:
            if (
                self._tip_left_local_idx is None
                or self._tip_right_local_idx is None
            ...
            left = int(self._tip_left_local_idx)
            right = int(self._tip_right_local_idx)
            if current_robot_points.shape[0] <= max(left, right):
                return None
            gripper_ref = 0.5 * (current_robot_points[left] + current_robot_points[right])
        return float(np.linalg.norm(bowl_points - gripper_ref[None, :], axis=-1).min())
    ...
    def _extract_eef_rotmat(self, raw_obs):
        ...
        if not self._warned_missing_eef_quat:
            print(
                "[libero_spatial] warning: missing eef orientation key, "
    ...
    def _build_default_robot_local_points(self):
        """
        Build canonical 9-point local offsets from the original Point-Policy
        Franka template (Tshift + extrapoints).
        """
    ...
    def _set_robot_local_geometry(self, robot_local_points, source):
        robot_local_points = np.asarray(robot_local_points, dtype=np.float32).reshape(-1, 3)
        if robot_local_points.shape[0] != len(ROBOT_SITE_SUFFIXES):
            raise ValueError(
                "robot_local_points must have 9 rows (kp0~kp8), "
                f"got {robot_local_points.shape[0]}"
            )
        self._robot_local_points_open = robot_local_points
        self._robot_tip_mid_local = 0.5 * (
            robot_local_points[1] + robot_local_points[2]
        )
        tip_delta = robot_local_points[1] - robot_local_points[2]
        tip_norm = float(np.linalg.norm(tip_delta))
        self._robot_tip_axis_local = (
            tip_delta / tip_norm if tip_norm > 1e-6 else np.array([0.0, 1.0, 0.0], dtype=np.float32)
        )
        # Keep close distance consistent with previous behavior while preserving
        # calibrated fingertip axis and midpoint.
        self._robot_tip_closed_half_gap = min(0.015, 0.5 * tip_norm) if tip_norm > 1e-6 else 0.015
        print(f"[libero_spatial] robot offsets source: {source}")

    def _maybe_calibrate_robot_offsets_from_sites(self, raw_obs):
        """
        For cal_offset modes, calibrate robot local offsets from simulator kp sites
        so runtime reconstruction matches the actual loaded gripper XML.
        """
        if self._robot_point_mode != "cal_offset":
            return
        if self._robot_offsets_calibrated_from_sites:
            return
        try:
            self._resolve_robot_site_ids_full()
            site_points = np.asarray(
                self._env.sim.data.site_xpos[self._robot_site_ids_full], dtype=np.float32
            ).reshape(-1, 3)
            eef_pos = self._extract_eef_pos(raw_obs)
            eef_rot = self._extract_eef_rotmat(raw_obs)
            local_points = (site_points - eef_pos[None, :]) @ eef_rot
            self._set_robot_local_geometry(local_points, source="site_calibrated")
            self._robot_offsets_calibrated_from_sites = True
        except Exception as exc:
            warnings.warn(
                "Failed to calibrate robot offsets from simulator sites; "
                f"falling back to canonical offsets. reason={exc}",
                RuntimeWarning,
            )
            self._robot_offsets_calibrated_from_sites = True
    ...
    def _resolve_robot_site_ids_full(self):
        self._refresh_name_caches()
        resolved_names = []
        missing = []
        for suffix in ROBOT_SITE_SUFFIXES:
        ...
        self._robot_site_ids_full = [
            self._site_name_to_id[name] for name in resolved_names
        ]

    def _resolve_robot_site_ids(self):
        if self._robot_site_ids_full is None:
            self._resolve_robot_site_ids_full()
        self._robot_site_ids = [
            self._robot_site_ids_full[idx] for idx in self._robot_point_indices
        ]

    def _resolve_object_names(self, raw_obs):
        if self._bowl_object_name is not None and self._plate_object_name is not None:
            return
        ...
        bowl_candidates = [name for name in obj_names if "bowl" in name]
        plate_candidates = [name for name in obj_names if "plate" in name]

        if len(bowl_candidates) > 0:
            self._bowl_object_name = bowl_candidates[0]
        if len(plate_candidates) > 0:
            self._plate_object_name = plate_candidates[0]

        if self._bowl_object_name is None or self._plate_object_name is None:
            raise ValueError(
                f"Could not resolve bowl/plate object names from obj_of_interest={obj_names}"
            )
    ...
    def _subtree_body_ids(self, root_body_id):
        body_parentid = np.asarray(self._env.sim.model.body_parentid, dtype=np.int32).reshape(-1)
        subtree = []
    ...
    def _collect_object_geom_ids(self, body_id):
        ...
        geom_ids = np.where(np.isin(geom_bodyid, subtree_body_ids))[0].astype(np.int32)
        if geom_ids.size == 0:
            raise ValueError(f"No geoms found for body subtree rooted at id={body_id}")
        return geom_ids

    def _select_representative_geom_ids(self, geom_ids, k):
        """
        Select k representative geoms from an object using FPS on current geom centers.
        Deterministic ordering helps keep point-track semantics stable.
        """
        geom_ids = np.asarray(geom_ids, dtype=np.int32).reshape(-1)
        if geom_ids.size == 0:
            raise ValueError("Cannot select representative geom ids from empty input")

        points = np.asarray(self._env.sim.data.geom_xpos[geom_ids], dtype=np.float32).reshape(-1, 3)
        if points.shape[0] <= k:
            selected_ids = geom_ids.copy()
            while selected_ids.shape[0] < k:
                selected_ids = np.concatenate([selected_ids, selected_ids[-1:]], axis=0)
            return selected_ids[:k]
        ...
        for _ in range(1, int(k)):
            ...
            dist_to_new = np.linalg.norm(points - points[nxt : nxt + 1], axis=-1)
    ...
    def _load_rim_bottom_local_points(self, object_category):
        xml_path = self._resolve_object_asset_xml(object_category)
        root = ET.parse(xml_path).getroot()
        bottom_site = root.find(".//site[@name='bottom_site']")
        top_site = root.find(".//site[@name='top_site']")
        radius_site = root.find(".//site[@name='horizontal_radius_site']")
        if bottom_site is None or top_site is None or radius_site is None:
            return RIM_BOTTOM_DEFAULT_LOCAL_POINTS.copy()

        bottom = _parse_vec(bottom_site.get("pos"), 3, [0.0, 0.0, -0.04])
        top = _parse_vec(top_site.get("pos"), 3, [0.0, 0.0, 0.02])
        radial = _parse_vec(radius_site.get("pos"), 3, [0.03, 0.0, 0.0])
        radius = float(np.linalg.norm(radial[:2]))
        if radius < 1e-6:
            radius = float(max(abs(radial[0]), abs(radial[1]), 0.03))

        z_rim = float(top[2])
        z_bottom = 0.0 if self._object_bottom_at_origin else float(bottom[2])
        return np.array(
            [
                [radius, 0.0, z_rim],
                [0.0, radius, z_rim],
    ...
    def _fit_point_count(self, points_local, k):
        points_local = np.asarray(points_local, dtype=np.float32).reshape(-1, 3)
        if points_local.shape[0] <= 0:
            raise ValueError("points_local must be non-empty")
        if points_local.shape[0] == int(k):
            return points_local.astype(np.float32)
        if points_local.shape[0] < int(k):
            out = points_local.copy()
            while out.shape[0] < int(k):
                out = np.concatenate([out, out[-1:]], axis=0)
            return out[: int(k)].astype(np.float32)

        centroid = points_local.mean(axis=0, keepdims=True)
        first = int(np.argmax(np.linalg.norm(points_local - centroid, axis=-1)))
        selected = [first]
        min_dist = np.linalg.norm(points_local - points_local[first : first + 1], axis=-1)
        for _ in range(1, int(k)):
            nxt = int(np.argmax(min_dist))
            selected.append(nxt)
            dist_to_new = np.linalg.norm(points_local - points_local[nxt : nxt + 1], axis=-1)
            min_dist = np.minimum(min_dist, dist_to_new)
        return points_local[np.asarray(selected, dtype=np.int32)].astype(np.float32)

    def _ensure_geometry_metadata(self, raw_obs):
        if (
            self._use_robot_points
            and self._robot_point_mode == "site"
            and self._robot_site_ids is None
        ):
            self._resolve_robot_site_ids()

        if self._use_object_points and (
            self._bowl_body_id is None
            or self._plate_body_id is None
            or (
                self._object_point_mode == "geom_fps"
                and (self._bowl_geom_ids is None or self._plate_geom_ids is None)
            )
            or (
                self._object_point_mode in {"cal_offset", "cal_offset_both"}
                and (self._bowl_local_points is None or self._plate_local_points is None)
            )
        ):
            self._resolve_object_names(raw_obs)
            self._bowl_body_id = self._resolve_body_id(self._bowl_object_name)
            self._plate_body_id = self._resolve_body_id(self._plate_object_name)
            if self._num_object_points % 2 != 0:
                raise ValueError(
                    f"num_object_points must be even (bowl + plate), got {self._num_object_points}"
                )
            per_object_points = self._num_object_points // 2
            if self._object_point_mode in {"cal_offset", "cal_offset_both"}:
                self._bowl_local_points = self._fit_point_count(
                    self._load_rim_bottom_local_points(
                        _instance_to_category(self._bowl_object_name)
                    ),
                    per_object_points,
                )
                self._plate_local_points = self._fit_point_count(
                    self._load_rim_bottom_local_points(
                        _instance_to_category(self._plate_object_name)
                    ),
                    per_object_points,
                )
                self._bowl_geom_ids = None
                ...
                bowl_geom_ids = self._collect_object_geom_ids(self._bowl_body_id)
                plate_geom_ids = self._collect_object_geom_ids(self._plate_body_id)
                self._bowl_geom_ids = self._select_representative_geom_ids(
                    bowl_geom_ids, per_object_points
                )
                self._plate_geom_ids = self._select_representative_geom_ids(
                    plate_geom_ids, per_object_points
                )
                self._bowl_local_points = None
                self._plate_local_points = None
    ...
    def _get_robot_points_from_pose(self, eef_pos, eef_rot, gripper_cmd):
        """
        Reconstruct 9 robot points from EEF pose + local offsets.
        Offsets are canonical by default, and are site-calibrated at runtime when
        available (for cal_offset modes).
        """
        local_points = np.asarray(self._robot_local_points_open, dtype=np.float32).copy()
        closed = float(gripper_cmd) > 0.0
        if closed and local_points.shape[0] >= 3:
            local_points[1] = (
    ...
    def _get_object_points_3d(self, raw_obs):
        ...
        if self._object_point_mode in {"cal_offset", "cal_offset_both"}:
            bowl_pos, bowl_quat = self._extract_body_pose(
                raw_obs, self._bowl_object_name, self._bowl_body_id
            )
            plate_pos, plate_quat = self._extract_body_pose(
                raw_obs, self._plate_object_name, self._plate_body_id
            )
            bowl_points = self._transform_local_points(
                bowl_pos, bowl_quat, np.asarray(self._bowl_local_points, dtype=np.float32)
            )
            plate_points = self._transform_local_points(
                plate_pos, plate_quat, np.asarray(self._plate_local_points, dtype=np.float32)
            )
        else:
            bowl_points = np.asarray(
                self._env.sim.data.geom_xpos[self._bowl_geom_ids], dtype=np.float32
            ).reshape(-1, 3)
            plate_points = np.asarray(
                self._env.sim.data.geom_xpos[self._plate_geom_ids], dtype=np.float32
            ).reshape(-1, 3)
        points = np.concatenate([bowl_points, plate_points], axis=0).astype(np.float32)
    ...
    def reset(self, **kwargs):
        ...
        self._maybe_calibrate_robot_offsets_from_sites(raw_obs)
    ...
    def point2action(self, action):
        self._point2action_stats["steps"] += 1
        ...
        if self._pose_solve_mode == "kp0":
            target_pos = target_robot_points[0]
            source_pos = current_robot_points[0]
            ...
            try:
                ...
                self._point2action_stats["rigid_fallback"] += 1
                target_pos = target_robot_points[0]
                delta_pos = target_pos - current_eef_pos
                delta_rot = np.zeros((3,), dtype=np.float32)

        # Optional gain to make approach motion less conservative at eval time.
        delta_pos = delta_pos * self._pose_delta_gain
        delta_pos_pre_clip = np.asarray(delta_pos, dtype=np.float32).copy()
        delta_rot_pre_clip = np.asarray(delta_rot, dtype=np.float32).copy()
        ...
        if np.any(np.abs(delta_pos_pre_clip) > (self._max_delta_pos + 1e-8)):
            self._point2action_stats["delta_clip_pos"] += 1
        if np.any(np.abs(delta_rot_pre_clip) > (self._max_delta_rot + 1e-8)):
            self._point2action_stats["delta_clip_rot"] += 1
            ...
            if (
                ...
                if pred_tip_distance is not None:
                    if (
                        ...
                        desired_cmd = -1.0
            # Keep gripper closed unless predicted tip distance is sufficiently large.
            # This suppresses premature reopen chatter while transporting an object.
            if self._gripper_reopen_tip_gate and prev_cmd > 0.0 and desired_cmd < 0.0:
                pred_tip_distance = self._tip_distance_from_points(target_robot_points)
                if (
                    pred_tip_distance is None
                    or pred_tip_distance < self._gripper_tip_open_threshold
                ):
                    desired_cmd = 1.0
        ...
        if self._gripper_close_position_gate and gripper_cmd > 0.0:
            if (
                ...
                gripper_cmd = -1.0
                self._gripper_hold_remaining = 0
        # Apply object proximity gate only when switching open->close.
        # Once closed, do not force reopen solely by this gate.
        if self._gripper_close_object_gate and gripper_cmd > 0.0 and prev_cmd < 0.0:
            bowl_dist = self._compute_gripper_to_bowl_distance(current_robot_points)
            if (
                bowl_dist is None
                or bowl_dist > self._gripper_close_object_threshold
            ):
                gripper_cmd = -1.0
                self._gripper_hold_remaining = 0
        ...
        # robosuite OSC_POSE expects normalized input in [-1, 1].
        # Convert meter / rotvec deltas into controller input scale when enabled.
        delta_pos_cmd = np.asarray(delta_pos, dtype=np.float32)
        delta_rot_cmd = np.asarray(delta_rot, dtype=np.float32)
        if self._normalize_delta_action_to_osc:
            delta_pos_scaled = delta_pos_cmd / self._controller_output_max_pos
            delta_rot_scaled = delta_rot_cmd / self._controller_output_max_rot
            if np.any(np.abs(delta_pos_scaled) > 1.0 + 1e-8):
                self._point2action_stats["osc_clip_pos"] += 1
            if np.any(np.abs(delta_rot_scaled) > 1.0 + 1e-8):
                self._point2action_stats["osc_clip_rot"] += 1
            delta_pos_cmd = np.clip(delta_pos_scaled, -1.0, 1.0)
            delta_rot_cmd = np.clip(delta_rot_scaled, -1.0, 1.0)

        robot_action = np.zeros((7,), dtype=np.float32)
        robot_action[:3] = delta_pos_cmd
        robot_action[3:6] = delta_rot_cmd
        robot_action[6] = gripper_cmd
        return robot_action

    def get_point2action_stats(self):
        return dict(self._point2action_stats)
...
def make(
    ...
    max_delta_rot,
    normalize_delta_action_to_osc,
    controller_output_max_pos,
    controller_output_max_rot,
    gripper_label_mode,
    gripper_threshold,
    ...
    if get_benchmark is None or get_libero_path is None or OffScreenRenderEnv is None:
        raise RuntimeError(
            "LIBERO import failed for libero_spatial suite. "
            f"Import error: {_LIBERO_IMPORT_ERROR}"
        )

    if not use_robot_points:
        raise ValueError("libero_spatial suite currently requires use_robot_points=true")
    if int(point_dim) != 3:
        raise ValueError("libero_spatial suite currently supports point_dim=3 only")
    ...
    for idx, task_key in enumerate(tasks):
        ...
        env = RGBArrayAsObservationWrapper(
            ...
            object_point_mode=object_point_mode,
            max_delta_pos=max_delta_pos,
            pose_delta_gain=pose_delta_gain,
            pose_solve_mode=pose_solve_mode,
            max_delta_rot=max_delta_rot,
            normalize_delta_action_to_osc=normalize_delta_action_to_osc,
            controller_output_max_pos=controller_output_max_pos,
            controller_output_max_rot=controller_output_max_rot,
            ...
            gripper_close_object_gate=gripper_close_object_gate,
            gripper_close_object_threshold=gripper_close_object_threshold,
            gripper_close_object_point=gripper_close_object_point,
            real_deploy_mode=real_deploy_mode,
            gripper_command_fallback_to_tip_distance=gripper_command_fallback_to_tip_distance,
            gripper_reopen_tip_gate=gripper_reopen_tip_gate,
            ...
            object_bottom_at_origin=object_bottom_at_origin,
            seed=seed + idx,
        )
