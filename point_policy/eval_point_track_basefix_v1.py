...
class Workspace:
    def __init__(self, cfg):
        ...
        self.point_debug_save_per_episode = bool(
            getattr(self.cfg, "point_debug_save_per_episode", True)
        )
        self.point_debug_save_aggregate = bool(
            getattr(self.cfg, "point_debug_save_aggregate", False)
        )
        self.emit_temporal_debug = bool(
            getattr(self.cfg.agent, "emit_temporal_debug", False)
        )
        self.temporal_debug_save_per_episode = bool(
            getattr(self.cfg, "temporal_debug_save_per_episode", True)
        )
        self.temporal_debug_save_aggregate = bool(
            getattr(self.cfg, "temporal_debug_save_aggregate", False)
        )
        ...
        self.contact_debug_save_per_episode = bool(
            getattr(self.cfg, "contact_debug_save_per_episode", True)
        )
        self.contact_debug_save_aggregate = bool(
            getattr(self.cfg, "contact_debug_save_aggregate", False)
        )
        self.point2action_stats_save_per_episode = bool(
            getattr(self.cfg, "point2action_stats_save_per_episode", True)
        )
        self.point2action_stats_save_aggregate = bool(
            getattr(self.cfg, "point2action_stats_save_aggregate", False)
        )
    ...
    def eval(self):
        ...
        for env_idx in range(self.envs_till_idx):
            ...
            success = []
            point_debug_inputs_all = []
            point_debug_preds_all = []
            point_debug_episodes_all = []
            point_debug_steps_all = []
            gripper_debug_pred_all = []
            gripper_debug_cmd_all = []
            gripper_debug_obs_all = []
            gripper_debug_qpos_all = []
            gripper_debug_episodes_all = []
            gripper_debug_steps_all = []
            temporal_debug_lines_all = []
            contact_debug_lines_all = []
            prev_p2a_stats = None
            debug_pixel_key = self.cfg.suite.pixel_keys[0]
            obs_point_key = f"point_tracks_{debug_pixel_key}"
            pred_point_key = f"future_tracks_{debug_pixel_key}"
            ...
            while eval_until_episode(episode):
                ...
                while True:
                    ...
                    if self.emit_temporal_debug and hasattr(
                        self.agent, "consume_temporal_debug"
                    ):
                        temporal_debug = self.agent.consume_temporal_debug()
                        if isinstance(temporal_debug, dict):
                            weights = temporal_debug.get("weights", [])
                            weight_preview = ",".join(
                                [f"{float(w):.4f}" for w in weights[:6]]
                            )
                            temporal_debug_lines.append(
                                " ".join(
                                    [
                                        f"ep={int(episode)}",
                                        f"step={int(step)}",
                                        f"pixel={temporal_debug.get('pixel_key', '-')}",
                                        f"num_candidates={int(temporal_debug.get('num_candidates', -1))}",
                                        f"weight_mode={temporal_debug.get('weight_mode', '-')}",
                                        f"weights=[{weight_preview}]",
                                    ]
                                )
                            )
                ...
                episode_idx_done = int(episode - 1)

                if (
                    self.point_debug
                    and self.point_debug_save_per_episode
                    and len(point_debug_inputs) > 0
                ):
                    point_debug_inputs_np = np.stack(point_debug_inputs, axis=0)
                    point_debug_preds_np = np.stack(point_debug_preds, axis=0)
                    debug_npz = (
                        self.work_dir
                        / f"point_debug_env{env_idx}_ep{episode_idx_done}.npz"
                    )
                    np.savez_compressed(
                        debug_npz,
                        input_points=point_debug_inputs_np,
                    ...
                    preview_txt = (
                        self.work_dir
                        / f"point_debug_env{env_idx}_ep{episode_idx_done}.txt"
                    )
                    with preview_txt.open("w", encoding="utf-8") as fp:
                        num_robot_points = int(self.cfg.suite.num_robot_points)
                        total_input_points = int(point_debug_inputs_np.shape[1])
                        ...
                        for idx in range(show_n):
                            object_preview_end = min(
                                total_input_points, num_robot_points + 3
                            )
                            fp.write(
                    ...
                    print(f"[point-debug][ep] saved: {debug_npz}")
                    print(f"[point-debug][ep] preview: {preview_txt}")

                if (
                    self.point_debug
                    and self.point_debug_save_per_episode
                    and len(gripper_debug_pred) > 0
                ):
                    gripper_txt = (
                        self.work_dir
                        / f"gripper_debug_env{env_idx}_ep{episode_idx_done}.txt"
                    )
                    ...
                    print(f"[gripper-debug][ep] saved: {gripper_txt}")

                if (
                    self.emit_temporal_debug
                    and self.temporal_debug_save_per_episode
                    and len(temporal_debug_lines) > 0
                ):
                    temporal_txt = (
                        self.work_dir
                        / f"temporal_debug_env{env_idx}_ep{episode_idx_done}.txt"
                    )
                    with temporal_txt.open("w", encoding="utf-8") as fp:
                        fp.write(
                            "Temporal aggregation debug\n"
                            "weights are applied to candidates for current step\n\n"
                        )
                        for line in temporal_debug_lines:
                            fp.write(line + "\n")
                    print(f"[temporal-debug][ep] saved: {temporal_txt}")

                if (
                    self.contact_debug
                    and self.contact_debug_save_per_episode
                    and len(contact_debug_lines) > 0
                ):
                    contact_txt = (
                        self.work_dir
                        / f"contact_debug_env{env_idx}_ep{episode_idx_done}.txt"
                    )
                    with contact_txt.open("w", encoding="utf-8") as fp:
                        ...
                        for line in contact_debug_lines:
                            fp.write(line + "\n")
                    print(f"[contact-debug][ep] saved: {contact_txt}")

                if (
                    self.point2action_stats_save_per_episode
                    and hasattr(self.env[env_idx], "get_point2action_stats")
                ):
                    try:
                        p2a_stats = self.env[env_idx].get_point2action_stats()
                    except Exception:
                        p2a_stats = None
                    if isinstance(p2a_stats, dict):
                        p2a_txt = (
                            self.work_dir
                            / f"point2action_stats_env{env_idx}_ep{episode_idx_done}.txt"
                        )
                        with p2a_txt.open("w", encoding="utf-8") as fp:
                            fp.write(f"episode={episode_idx_done}\n")
                            fp.write("[current]\n")
                            for key in sorted(p2a_stats.keys()):
                                fp.write(f"{key}={p2a_stats[key]}\n")
                            if isinstance(prev_p2a_stats, dict):
                                fp.write("\n[delta_from_prev_episode]\n")
                                for key in sorted(p2a_stats.keys()):
                                    cur_val = p2a_stats[key]
                                    prev_val = prev_p2a_stats.get(key, 0.0)
                                    try:
                                        delta_val = float(cur_val) - float(prev_val)
                                        fp.write(f"{key}={delta_val}\n")
                                    except Exception:
                                        continue
                        print(f"[point2action-stats][ep] saved: {p2a_txt}")
                        prev_p2a_stats = dict(p2a_stats)

                if self.point_debug_save_aggregate:
                    point_debug_inputs_all.extend(point_debug_inputs)
                    point_debug_preds_all.extend(point_debug_preds)
                    point_debug_episodes_all.extend(point_debug_episodes)
                    point_debug_steps_all.extend(point_debug_steps)
                    gripper_debug_pred_all.extend(gripper_debug_pred)
                    gripper_debug_cmd_all.extend(gripper_debug_cmd)
                    gripper_debug_obs_all.extend(gripper_debug_obs)
                    gripper_debug_qpos_all.extend(gripper_debug_qpos)
                    gripper_debug_episodes_all.extend(gripper_debug_episodes)
                    gripper_debug_steps_all.extend(gripper_debug_steps)
                if self.emit_temporal_debug and self.temporal_debug_save_aggregate:
                    temporal_debug_lines_all.extend(temporal_debug_lines)
                if self.contact_debug and self.contact_debug_save_aggregate:
                    contact_debug_lines_all.extend(contact_debug_lines)
            ...
            if (
                self.point_debug
                and self.point_debug_save_aggregate
                and len(point_debug_inputs_all) > 0
            ):
                point_debug_inputs_np = np.stack(point_debug_inputs_all, axis=0)
                point_debug_preds_np = np.stack(point_debug_preds_all, axis=0)
                debug_npz = self.work_dir / f"point_debug_env{env_idx}.npz"
                np.savez_compressed(
                    debug_npz,
                    input_points=point_debug_inputs_np,
                    pred_points=point_debug_preds_np,
                    episode=np.asarray(point_debug_episodes_all, dtype=np.int32),
                    step=np.asarray(point_debug_steps_all, dtype=np.int32),
                    pixel_key=np.asarray([debug_pixel_key]),
                )
                ...
                with preview_txt.open("w", encoding="utf-8") as fp:
                    ...
                    fp.write(
                        f"pixel_key={debug_pixel_key}\n"
                        f"num_samples={len(point_debug_inputs_all)}\n"
                        f"input_points shape: [num_samples, {total_input_points}, {input_dim}] "
                        f"(robot {num_robot_points} + object {num_object_points})\n"
                        f"pred_points shape: [num_samples, {pred_points_count}, {pred_dim}] "
                        "(robot only)\n\n"
                    )
                    show_n = min(3, len(point_debug_inputs_all))
                    for idx in range(show_n):
                        object_preview_end = min(total_input_points, num_robot_points + 3)
                        fp.write(
                            f"[sample {idx}] episode={point_debug_episodes_all[idx]} "
                            f"step={point_debug_steps_all[idx]}\n"
                        )
                        fp.write(
                            "input_robot_first3=\n"
                            f"{point_debug_inputs_np[idx, :3]}\n"
            ...
            if (
                self.point_debug
                and self.point_debug_save_aggregate
                and len(gripper_debug_pred_all) > 0
            ):
                ...
                with gripper_txt.open("w", encoding="utf-8") as fp:
                    ...
                    for idx in range(len(gripper_debug_pred_all)):
                        fp.write(
                            f"ep={gripper_debug_episodes_all[idx]} "
                            f"step={gripper_debug_steps_all[idx]} "
                            f"obs_gripper={gripper_debug_obs_all[idx]:.6f} "
                            f"raw_qpos={gripper_debug_qpos_all[idx]:.6f} "
                            f"pred_gripper={gripper_debug_pred_all[idx]:.6f} "
                            f"cmd_gripper={gripper_debug_cmd_all[idx]:.1f}\n"
                        )
                print(f"[gripper-debug] saved: {gripper_txt}")
            if (
                self.emit_temporal_debug
                and self.temporal_debug_save_aggregate
                and len(temporal_debug_lines_all) > 0
            ):
                temporal_txt = self.work_dir / f"temporal_debug_env{env_idx}.txt"
                with temporal_txt.open("w", encoding="utf-8") as fp:
                    fp.write(
                        "Temporal aggregation debug\n"
                        "weights are applied to candidates for current step\n\n"
                    )
                    for line in temporal_debug_lines_all:
                        fp.write(line + "\n")
                print(f"[temporal-debug] saved: {temporal_txt}")
            if (
                self.point2action_stats_save_aggregate
                and hasattr(self.env[env_idx], "get_point2action_stats")
            ):
                try:
                    p2a_stats = self.env[env_idx].get_point2action_stats()
                    ...
                    p2a_stats = None
                if isinstance(p2a_stats, dict):
                    p2a_txt = self.work_dir / f"point2action_stats_env{env_idx}.txt"
                    with p2a_txt.open("w", encoding="utf-8") as fp:
                        for key in sorted(p2a_stats.keys()):
                            fp.write(f"{key}={p2a_stats[key]}\n")
                    print(f"[point2action-stats] saved: {p2a_txt}")
            if (
                self.contact_debug
                and self.contact_debug_save_aggregate
                and len(contact_debug_lines_all) > 0
            ):
                ...
                with contact_txt.open("w", encoding="utf-8") as fp:
                    ...
                    for line in contact_debug_lines_all:
                        fp.write(line + "\n")
